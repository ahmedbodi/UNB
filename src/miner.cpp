...
{
    LogPrintf("VoxelsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("voxels-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running VoxelsMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("VoxelsMiner terminated\n");
        throw;
    }
}

void GenerateVoxelss(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VoxelsMiner, pwallet));
}
