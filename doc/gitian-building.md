...
*Setup instructions for a gitian build of Voxels using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Voxels
...
- [Building Voxels](#building-voxels)
...
Clone the git repositories for voxels and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/voxels/voxels
```
...
In [doc/release-process.md](release-process.md) in the voxels repository under 'Fetch and build inputs'.
...
To build Voxels (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the voxels repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/voxels/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/voxels/voxels
...
For example:
```bash
URL=https://github.com/laanwj/voxels.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit voxels=${COMMIT} --url voxels=${URL} ../voxels/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit voxels=${COMMIT} --url voxels=${URL} ../voxels/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/voxels-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/voxels-build.assert
```
...
[voxels/gitian.sigs](https://github.com/voxels/gitian.sigs/) repository, or
