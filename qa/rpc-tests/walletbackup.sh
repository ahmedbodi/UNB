...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} vox"
...
function StartThree {
  $VOXELSD $VOXELSDARGS $B1ARGS &
  B1PID=$!
  $VOXELSD $VOXELSDARGS $B2ARGS &
  B2PID=$!
  $VOXELSD $VOXELSDARGS $B3ARGS &
  B3PID=$!
}
